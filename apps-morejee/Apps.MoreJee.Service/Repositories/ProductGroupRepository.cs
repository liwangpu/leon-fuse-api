...
namespace Apps.MoreJee.Service.Repositories
{
    public class ProductGroupRepository : IRepository<ProductGroup>
    {
        ...
        public async Task<string> CanCreateAsync(ProductGroup data, string accountId)
        {
        ...
        public async Task<string> CanUpdateAsync(ProductGroup data, string accountId)
        {
            return await Task.FromResult(string.Empty);
        }

        public async Task CreateAsync(ProductGroup data, string accountId)
        {
        ...
        {
            var entity = await _Context.ProductGroups.FirstOrDefaultAsync(x => x.Id == id);
            if (entity != null)
            {
        ...
        public async Task<ProductGroup> GetByIdAsync(string id, string accountId)
        {
            var entity = await _Context.ProductGroups.FirstOrDefaultAsync(x => x.Id == id);
            return entity;
        }

        public async Task<PagedData<ProductGroup>> SimplePagedQueryAsync(PagingRequestModel model, string accountId, Func<IQueryable<ProductGroup>, Task<IQueryable<ProductGroup>>> advanceQuery = null)
        {
        ...
        public async Task UpdateAsync(ProductGroup data, string accountId)
        {
            data.Modifier = accountId;
            data.ModifiedTime = data.CreatedTime;
            _Context.ProductGroups.Update(data);
            await _Context.SaveChangesAsync();
        }
    }
}
