...
namespace DataTransferHelper
{
    class Program
    {
        ...
        static async Task TransferData()
        {
            ...
            {
                var catIds = new List<string> { "ZRCNG7JQAR026Q", "JQCN5659NX0GX1", "KEC9M6GZQV39VV" };
                var totals = destDb.Set<AssetCategory>().Where(x => x.Type == "PanelProduct").Count();
            ...
            var bamboo_admin = "VMU79E35V6KAYN";
            var bamboo_organId = "9VUA395Z0WEN38";
            var ehome_admin = "17UP463K965V6G";
            var echome_organId = "8WU9G4K0WWP0ME";
            ...
            using (var destDb = new DestContext())
            {
                ...
                //5=>分类
                {
                    var tranferCat = "order";
                    var orgMatCats = destDb.AssetCategories.Where(x => x.Type == tranferCat).ToList();
                    destDb.AssetCategories.RemoveRange(orgMatCats);
                    var orgMatTree = destDb.AssetCategoryTrees.Where(x => x.NodeType == tranferCat).ToList();
                    destDb.AssetCategoryTrees.RemoveRange(orgMatTree);
                    destDb.SaveChanges();
                    //cat
                    {
                        var srcIds = srcDb.AssetCategories.Where(x => x.OrganizationId == bamboo_organId && x.Type == tranferCat).Select(x => x.Id).ToList();
                        var len = srcIds.Count;
                        for (int i = 0; i < len; i++)
                        {
                            var sid = srcIds[i];
                    ...
                    //cat tree
                    {
                        var srcIds = srcDb.AssetCategoryTrees.Where(x => x.OrganizationId == bamboo_organId && x.NodeType == tranferCat).Select(x => x.Id).ToList();
                        var len = srcIds.Count;
                        for (int i = 0; i < len; i++)
                        {
                            var sid = srcIds[i];
        ...
        /// </summary>
        /// <returns></returns>
        static async Task RebuildCategoryTree()
        {
            using (var destDb = new DestContext())
            {
